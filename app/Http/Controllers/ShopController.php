...
class ShopController extends Controller
{
        ...
        public function index(Request $request)
    {
        ...
        $min_price = $request->query('min') ? $request->query('min') : 1;
        $max_price = $request->query('max') ? $request->query('max') : 1000;
        $query = $request->query('query');  // Ambil kata kunci pencarian
        ...
        $products = Product::where(function($query) use($f_brands) {
            ...
            // Menambahkan pencarian berdasarkan nama produk jika ada query pencarian
            ->when($query, function($searchQuery) use ($query) {
                return $searchQuery->where('name', 'LIKE', "%{$query}%");
            })
            ->orderBy($o_column, $o_order)
            ->paginate($size);

            return view('shop', compact('products', 'size', 'order', 'brands', 'f_brands', 'categories', 'f_categories', 'min_price', 'max_price', 'query'));
        }
