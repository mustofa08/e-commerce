                ...
                <ul class="list list-inline mb-0">
                  ...
                  <li class="list-item">
                    <span class="menu-link py-1">
                      <input class="chk-category" type="checkbox" name="categories" value="{{$category->id}}" 
                  ...
                  <div class="product-card-wrapper">
                      <div class="product-card mb-3 mb-md-4 mb-xxl-5">
                          ...
                          <div class="pc__info position-relative">
                              ...
                              <form method="POST" action="{{route('whistlist.item.remove',['rowId'=>Cart::instance('whislist')->content()->where('id',$product->id)->first()->rowId])}}">
                                @csrf
                                @method('DELETE')
...
<script>
    $(function() {
        // Event ketika ukuran halaman berubah
        $("#pagesize").on("change", function() {
            $("#size").val($("#pagesize option:selected").val());
            updateAndReload();
        });

        // Event ketika urutan berubah
        $("#orderby").on("change", function() {
            $("#order").val($("#orderby option:selected").val());
            updateAndReload();
        });

        // Event ketika filter merek berubah
        ...
        // Fungsi untuk memperbarui URL dan memuat ulang halaman
        function updateAndReload() {
            const page = $("#page").val();
            const size = $("#size").val();
            const order = $("#order").val();
            const brands = $("#hdnBrands").val();
            const categories = $("#hdnCategories").val();
            const min = $("#hdnMinPrice").val();
            const max = $("#hdnMaxPrice").val();

            // Bangun URL baru
            const newUrl = `?page=${page}&size=${size}&order=${order}&brands=${brands}&categories=${categories}&min=${min}&max=${max}`;
            
            // Perbarui URL tanpa reload langsung
